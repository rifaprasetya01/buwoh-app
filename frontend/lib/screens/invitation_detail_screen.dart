...
import '../services/buwoh_service.dart';
...
class _InvitationDetailScreenState extends State<InvitationDetailScreen> {
  ...
  void _ajukanBuwoh() async {
    ...
    if (_uang) gifts.add({'giftCategoryId': 3, 'quantity': 1});
    if (_beras) gifts.add({'giftCategoryId': 1, 'quantity': 1});
    if (_gula) gifts.add({'giftCategoryId': 2, 'quantity': 1});

    if (widget.eventId != null) {
      try {
        await BuwohService.applyAsGuest(
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Gagal mengajukan buwoh. Periksa koneksi.'),
